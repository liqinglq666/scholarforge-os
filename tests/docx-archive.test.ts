...
function fakeArchive(entries: EntrySpec[], totalEntriesOverride?: number) {
  ...
  return concat([centralDirectory, eocd]).buffer as ArrayBuffer;
}
...
describe('DOCX ZIP archive preflight', () => {
  ...
  it('accepts a real DOCX generated by the current export dependency', async () => {
    const document = new Document({
      sections: [{ children: [new Paragraph('ScholarForge DOCX preflight integration test.')] }],
    });
    ...
    const result = inspectDocxArchive(arrayBuffer);

    expect(result.entryCount).toBeGreaterThan(2);
